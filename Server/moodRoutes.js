...
const getMood = async (req, res, next) => {
  try {
    ...
    if (!mood) {
      return res.status(404).json({ message: 'Mood entry not found' }); //data not found
    }
    res.mood = mood;
    next();
  } catch (error) {
...
router.post('/', validateMoodEntry, async (req, res) => {
  ...
  try {
    ...
    res.status(201).json(newMood);//data added 
  } catch (error) {
...
router.get('/summary/:date', async (req, res) => {
  try {
    ...
    return res.status(400).json({ message: "Invalid date format. Use YYYY-MM-DD format." }); //bad request
    }
